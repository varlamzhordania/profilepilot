...
export async function getOrCreateCreditAccountDoc(uid: string) {
    ...
    try {
        ...
        // Auto-reconcile pending or uncredited paid orders for this UID across both collections
        try {
            let updatedBalance = resultAccount.balance;
            let updatedPurchased = resultAccount.totalPurchased;

            // 1. Check New Universal Orders Collection
            const pendingOrdersSnap = await adminDb.collection("orders").where("uid", "==", uid).get();
            for (const orderDoc of pendingOrdersSnap.docs) {
                const orderData = orderDoc.data();
                if (orderData.status === "paid" && orderData.credited !== true && orderData.gatewayPaymentId) {
                    const fulfillRes = await fulfillOrder(orderDoc.id, orderData.gatewayPaymentId, orderData.gateway || "razorpay", uid);
            ...
            // 2. Check Legacy Razorpay Orders Collection
            const legacyOrdersSnap = await adminDb.collection("razorpayOrders").where("uid", "==", uid).get();
            for (const orderDoc of legacyOrdersSnap.docs) {
                const orderData = orderDoc.data();
                if (orderData.status === "paid" && orderData.credited !== true && orderData.razorpayPaymentId) {
                    const fulfillRes = await fulfillOrder(orderDoc.id, orderData.razorpayPaymentId, "razorpay", uid);
                    if (fulfillRes.success) {
                        updatedBalance = fulfillRes.newBalance;
                        updatedPurchased += (fulfillRes.creditsAdded || 0);
                    }
    ...
    await adminDb.runTransaction(async (transaction) => {
      ...
      try {
        ...
        transaction.set(txRef, {
          ...
          razorpayOrderId: null, // Kept for legacy compatibility
          razorpayPaymentId: null, // Kept for legacy compatibility
...
export async function fulfillOrder(
  ...
  try {
    await adminDb.runTransaction(async (transaction) => {
      ...
      let orderSnap = await transaction.get(universalOrderRef);
      let activeOrderRef = universalOrderRef;

      if (!orderSnap.exists) {
        orderSnap = await transaction.get(legacyOrderRef);
        activeOrderRef = legacyOrderRef;
      }

      if (!orderSnap.exists) {
        throw new Error(`ORDER_NOT_FOUND: Order ${orderId} does not exist in server records.`);
      }
      ...
      transaction.set(paymentTxRef, {
        ...
        gatewayPaymentId: gatewayPaymentId,
        razorpayOrderId: gateway === 'razorpay' ? orderId : null,
        razorpayPaymentId: gateway === 'razorpay' ? gatewayPaymentId : null,
        referenceId: `${gateway}_payment_${gatewayPaymentId}`,
        createdAt: FieldValue.serverTimestamp(),
      });
    });
    ...
    if (fallbackOrder) {
      ...
      fallbackOrder.gatewayPaymentId = gatewayPaymentId;
    }

    if (!isAlreadyProcessed) {
      logTelemetry("credit_purchased", `Order ${orderId} (${gateway}) fulfilled! Added +${creditsAdded} credits to UID ${authenticatedUid || 'webhook'}.`);
    }
    ...
    console.error("[fulfillOrder Error]", err?.message || err);
    const errorMsg = err?.message || String(err);

    if (errorMsg.includes("This payment belongs to a different ProfilePilot account")) {
      return { success: false, creditsAdded: 0, newBalance: 0, error: errorMsg };
    }
    ...
    if (fallbackOrder) {
      ...
      if (fallbackOrder.credited === true || fallbackAcc.lastFulfilledPaymentId === gatewayPaymentId) {
        return { success: true, creditsAdded: 0, newBalance: fallbackAcc.balance, alreadyProcessed: true };
      }
      ...
      fallbackAcc.lastFulfilledPaymentId = gatewayPaymentId;
      fallbackAcc.balance += creditsToAdd;
      fallbackAcc.totalPurchased += creditsToAdd;
      fallbackCreditAccounts.set(targetUid, fallbackAcc);

      logTelemetry("credit_purchased", `Order ${orderId} fulfilled via fallback store! Added +${creditsToAdd} credits to UID ${targetUid}.`);

      return { success: true, creditsAdded: creditsToAdd, newBalance: fallbackAcc.balance };
    }
...
app.post("/api/credits/purchase", requireAuth, async (req, res) => {
    ...
    try {
        ...
        const acc = fallbackCreditAccounts.get(authUser.uid) || {
            uid: authUser.uid,
            balance: 30,
            totalPurchased: 0,
            totalUsed: 0
...
// ================= PAYMENT GENERATION & VERIFICATION =================
...
const PAYPAL_SUPPORTED_CURRENCIES = new Set(["USD", "EUR", "GBP", "CAD", "AUD"]);
...
async function getPayPalAccessToken(config: NonNullable<ReturnType<typeof getPayPalConfig>>): Promise<string> {
  ...
  if (!res.ok) throw new Error(`PayPal token error: ${data.error_description || data.error || "Unknown"}`);
...
const handleCreateOrder = async (req: express.Request, res: express.Response) => {
  try {
    const authUser = getAuthUser(req);
    const { packageId: reqPackageId, packId, currency: reqCurrency = "INR", gateway = "razorpay" } = req.body;
    const requestedId = reqPackageId || packId;

    if (requestedId && !SERVER_PACKAGE_MAP[requestedId]) {
      return res.status(400).json({ success: false, error: "InvalidPackage", message: "Invalid credit package selected." });
    }
    ...
    if (gateway === "razorpay") {
      ...
      if (!rzpConfig) {
        return res.status(500).json({ success: false, error: "ConfigError", message: "We could not start the payment securely." });
      }

      const receiptId = `rcpt_${Date.now()}_${Math.random().toString(36).substring(2, 6)}`;
      const rzpOrder = await rzpConfig.instance.orders.create({
        amount: amountInSubunits,
        currency: selectedCurrency,
        receipt: receiptId,
        notes: { uid: authUser.uid, packageId: targetPackageId },
      });
      ...
      const ppConfig = getPayPalConfig();
      if (!ppConfig) {
        return res.status(500).json({ success: false, error: "ConfigError", message: "PayPal is not configured on this server." });
      }

      // PayPal doesn't support INR – fall back to USD
      const paypalCurrency = PAYPAL_SUPPORTED_CURRENCIES.has(selectedCurrency) ? selectedCurrency : "USD";
      const paypalPriceInfo = pkg.prices[paypalCurrency] || pkg.prices["USD"];
      const paypalAmount = paypalPriceInfo.price.toFixed(2);

      let accessToken: string;
      try {
        accessToken = await getPayPalAccessToken(ppConfig);
      } catch (tokenErr: any) {
        console.error("[PAYPAL_TOKEN_ERROR]", tokenErr?.message);
        return res.status(500).json({ success: false, error: "PayPalConfigError", message: "Could not authenticate with PayPal." });
      }

      const ppRes = await fetch(`${ppConfig.baseUrl}/v2/checkout/orders`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${accessToken}`,
          "Content-Type": "application/json",
          "PayPal-Request-Id": `pp_${authUser.uid}_${Date.now()}`,
        },
        body: JSON.stringify({
          intent: "CAPTURE",
          purchase_units: [{
            reference_id: `pp_${targetPackageId}_${authUser.uid}`,
            description: `ProfilePilot – ${pkg.credits} AI Credits`,
            amount: {
      ...
      if (!ppRes.ok || !ppOrder.id) {
        console.error("[PAYPAL_ORDER_CREATE_ERROR]", ppOrder);
        return res.status(500).json({ success: false, error: "PayPalApiError", message: ppOrder.message || "PayPal order creation failed." });
      }
      ...
      return res.status(400).json({ success: false, error: "UnsupportedGateway", message: "Gateway not supported." });
    }


    // NEW SCHEMA: Store Order in the Universal 'orders' collection
    const orderDocData = {
      uid: authUser.uid,
      ...
      gateway: gateway,
      gatewayOrderId: orderId,
      razorpayOrderId: gateway === "razorpay" ? orderId : null, // For backward frontend compatibility
      status: "created",
      credited: false,
      gatewayPaymentId: null,
    ...
    fallbackOrders.set(orderId, orderDocData);
    ...
    try {
      ...
      console.warn(`[FIRESTORE_ORDER_SAVE_WARN] Could not write order ${orderId} to Firestore. Saved to fallback store.`);
    }

    logTelemetry("credit_purchased", `Order Created: ${orderId} via ${gateway} for UID ${authUser.uid}`);
    ...
    console.error("[ORDER_CREATE_FAILED] Order Creation Error:", err?.message || err);
    return res.status(500).json({ success: false, error: "ApiError", message: "We could not start the payment securely." });
  }
};
...
const handleVerifyPayment = async (req: express.Request, res: express.Response) => {
  try {
    const authUser = getAuthUser(req);
    const { razorpay_payment_id, razorpay_order_id, razorpay_signature, gatewayPaymentId, gatewayOrderId, gateway = "razorpay" } = req.body;

    const finalOrderId = gatewayOrderId || razorpay_order_id;
    const finalPaymentId = gatewayPaymentId || razorpay_payment_id;

    if (!finalOrderId || !finalPaymentId) {
      return res.status(400).json({ success: false, error: "MissingFields", message: "Missing required verification fields." });
    }

    if (gateway === "razorpay") {
      const rzpConfig = getRazorpayInstance();
      if (!rzpConfig || !razorpay_signature) return res.status(400).json({ success: false, error: "InvalidSignature", message: "Verification failed." });

      const body = `${finalOrderId}|${finalPaymentId}`;
      const expectedSignature = crypto.createHmac("sha256", rzpConfig.keySecret).update(body).digest("hex");
      const isSignatureValid = crypto.timingSafeEqual(Buffer.from(expectedSignature, "utf-8"), Buffer.from(razorpay_signature, "utf-8"));

      if (!isSignatureValid) {
        return res.status(400).json({ success: false, error: "InvalidSignature", message: "Security signature match failed." });
      }
    }

    // Fulfill the order transparently - this will automatically search both the universal and legacy collections!
    const fulfillment = await fulfillOrder(finalOrderId, finalPaymentId, gateway, authUser.uid);

    if (!fulfillment.success) {
      return res.status(400).json({ success: false, error: "FulfillmentError", message: fulfillment.error || "Failed to add credits." });
    }
    ...
    return res.status(500).json({ success: false, error: "VerificationFailed", message: err?.message || "Error verifying payment." });
  }
};

const handlePayPalCapture = async (req: express.Request, res: express.Response) => {
  try {
    const authUser = getAuthUser(req);
    const { paypalOrderId } = req.body;

    if (!paypalOrderId) {
      return res.status(400).json({ success: false, error: "MissingFields", message: "Missing paypalOrderId." });
    }

    const ppConfig = getPayPalConfig();
    if (!ppConfig) {
      return res.status(500).json({ success: false, error: "ConfigError", message: "PayPal is not configured." });
    }
    ...
    const captureRes = await fetch(`${ppConfig.baseUrl}/v2/checkout/orders/${paypalOrderId}/capture`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
    ...
    if (!captureRes.ok || captureData.status !== "COMPLETED") {
      console.error("[PAYPAL_CAPTURE_ERROR]", captureData);
      return res.status(400).json({
        success: false,
        error: "CaptureFailed",
        message: captureData.message || `PayPal payment not completed (status: ${captureData.status}).`,
      });
    }

    const captureId = captureData.purchase_units?.[0]?.payments?.captures?.[0]?.id
      || `pp_capture_${Date.now()}`;

    const fulfillment = await fulfillOrder(paypalOrderId, captureId, "paypal", authUser.uid);

    if (!fulfillment.success) {
      return res.status(400).json({ success: false, error: "FulfillmentError", message: fulfillment.error || "Credits could not be added." });
    }

    logTelemetry("credit_purchased", `PayPal captured: ${paypalOrderId} / ${captureId} → UID ${authUser.uid} +${fulfillment.creditsAdded} credits`);

    return res.json({
      success: true,
      message: "PayPal payment captured and credits added!",
      creditsAdded: fulfillment.creditsAdded,
      newBalance: fulfillment.newBalance,
      alreadyProcessed: fulfillment.alreadyProcessed || false,
    });
  } catch (err: any) {
    console.error("[PAYPAL_CAPTURE_ERROR]", err?.message || err);
    return res.status(500).json({ success: false, error: "CaptureError", message: err?.message || "PayPal capture failed." });
  }
};
  ...
  try {
    ...
    if (eventType === "PAYMENT.CAPTURE.COMPLETED") {
      ...
      if (orderId && captureId) {
        const fulfillRes = await fulfillOrder(orderId, captureId, "paypal");
        console.log(`[PayPal Webhook] PAYMENT.CAPTURE.COMPLETED: ${orderId} →`, fulfillRes);
      }
    }

    res.json({ status: "ok" });
  } catch (err: any) {
    console.error("[PAYPAL_WEBHOOK_ERROR]", err?.message || err);
    res.status(500).json({ error: "WebhookError" });
  }
});
app.post("/api/payments/razorpay/webhook", async (req, res) => {
  ...
  if (!webhookSignature) return res.status(400).json({ error: "MissingSignature" });

  try {
    const bodyStr = JSON.stringify(req.body);
    const expectedSignature = crypto.createHmac("sha256", webhookSecret || '').update(bodyStr).digest("hex");

    if (expectedSignature !== webhookSignature) return res.status(400).json({ error: "InvalidSignature" });
    ...
    if (event === "payment.captured" || event === "order.paid") {
      ...
      if (orderId && paymentId) {
        const fulfillRes = await fulfillOrder(orderId, paymentId, "razorpay");
        console.log(`[Webhook] Fulfill result for ${orderId}:`, fulfillRes);
      }
    }
    res.json({ status: "ok" });
  } catch (err: any) {
...
app.post("/api/payments/paypal/capture-order", requireAuth, handlePayPalCapture);
...
app.post("/api/admin/reconcile-payments", optionalAuth, async (req, res) => {
  ...
  if (adminKey !== (process.env.ADMIN_SECRET_KEY || "profilepilot_admin_secret_2026")) {
  ...
  try {
    ...
    ordersSnap.forEach((doc) => {
      const d = doc.data();
      if (d.credited === true) creditedCount++;
      else if (d.status === "paid") {
        uncreditedPaidCount++;
        reportList.push({ orderId: doc.id, uid: d.uid, creditsToAdd: d.creditsToAdd, paymentId: d.gatewayPaymentId });
      }
    });

    // Check Legacy Orders
    const legacySnap = await adminDb.collection("razorpayOrders").get();
    legacySnap.forEach((doc) => {
      const d = doc.data();
      if (d.credited === true) creditedCount++;
      else if (d.status === "paid") {
        uncreditedPaidCount++;
        reportList.push({ orderId: doc.id, uid: d.uid, creditsToAdd: d.creditsToAdd, paymentId: d.razorpayPaymentId });
      }
    });

    res.json({
      success: true,
      totalOrders: ordersSnap.size + legacySnap.size,
      creditedCount,
      uncreditedPaidCount,
...
app.post("/api/admin/attach-payment", optionalAuth, async (req, res) => {
  const adminKey = req.headers["x-admin-key"] || req.query.adminKey || req.body?.adminKey;
  if (adminKey !== (process.env.ADMIN_SECRET_KEY || "profilepilot_admin_secret_2026")) return res.status(403).json({ error: "Forbidden" });

  const { razorpayOrderId, razorpayPaymentId, gatewayOrderId, gatewayPaymentId, gateway = "razorpay", targetUid } = req.body;
  const finalOrderId = gatewayOrderId || razorpayOrderId;
  const finalPaymentId = gatewayPaymentId || razorpayPaymentId || `manual_${Date.now()}`;

  try {
    try {
      // First check universal collection, then fallback to legacy
      let orderRef = adminDb.collection("orders").doc(finalOrderId);
      let orderSnap = await orderRef.get();

      if (!orderSnap.exists) {
        orderRef = adminDb.collection("razorpayOrders").doc(finalOrderId);
      ...
      if (orderSnap.exists) {
        ...
        await adminDb.collection("orders").doc(finalOrderId).set({
          uid: targetUid,
    ...
    } catch (fsErr) { /* Skip if offline */ }

    const fulfillRes = await fulfillOrder(finalOrderId, finalPaymentId, gateway, targetUid);
...
// Fallback error handlers and remaining endpoints (AI, transcription, static serving, etc.) stay functionally identical here
app.all("/api/*", (req, res) => {
  res.status(404).json({ error: "NotFound", message: `API route ${req.method} ${req.path} not found.` });
});
...
app.post("/api/coach-chat", requireAuth, async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            error: "CoachChatFailed",
            message: "Failed to process dating coach response. Your credit has been refunded."
        });
    }
});
...
4. Long-Term Positioning Strategy`;
        ...
        await refundFn();
        res.status(500).json({
            error: "DeepAuditFailed",
            message: "Failed to complete high-thinking audit. Credit refunded."
        });
    }
});
...
app.post("/api/profile/ai-feedback", requireAuth, async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            ...
            message: "Failed to process profile feedback. Credit refunded."
        });
    }
});
