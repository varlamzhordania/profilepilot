...
import { Coins, Sparkles, Check, CreditCard, Lock, Zap, X, Globe2, ShieldCheck, ArrowRight, ArrowLeft, Code, Terminal, ExternalLink, QrCode, UserCheck, UserPlus, History, HelpCircle } from 'lucide-react';
...
export const PAYMENT_GATEWAYS: PaymentGateway[] = [
  { id: 'razorpay', name: 'Razorpay / UPI / GPay', badge: 'GPay / PhonePe / Paytm / Cards', description: 'Instant UPI, QR Code & Indian/Global Bank Cards' },
];
...
export interface CreditPack {
  ...
  description: string;
  usageExample: string;
  prices: Record<string, { price: number; formatted: string }>;
}

export const BASE_CREDIT_PACKS: CreditPack[] = [
  {
    id: 'starter_refill',
...
export const CreditModal: React.FC<CreditModalProps> = ({
  ...
  const fetchCreditHistory = async () => {
    ...
    try {
      const { ok, data } = await safeFetchJson<{ success?: boolean; history?: CreditTransaction[] }>('/api/credits/history');
  ...
  const currentPack = BASE_CREDIT_PACKS.find((p) => p.id === selectedPackId) || BASE_CREDIT_PACKS[1];
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden my-8 animate-fadeIn text-slate-100">
        ...
        <div className="bg-gradient-to-r from-orange-950/60 via-slate-900 to-pink-950/60 border-b border-slate-800 p-6 relative">
          ...
          <div className="mt-4 p-3 bg-slate-950/80 border border-slate-800/80 rounded-2xl flex items-center justify-between text-xs gap-2 flex-wrap">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {BASE_CREDIT_PACKS.map((pack) => {
                    ...
                    return (
                      ...
                      >
                        {pack.badge && (
                          <span className={`absolute -top-2.5 right-4 px-2.5 py-0.5 text-[10px] font-extrabold rounded-full border shadow-sm ${
                            pack.popular
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-3">
                            <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors mt-0.5 shrink-0 ${
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      setGateway(gw);
                      if (gw.id === 'razorpay') {
                        setCurrency(SUPPORTED_CURRENCIES[0]); // INR
                      }
                    }}
          ...
          {gateway.id === 'razorpay' && (
            <div className="p-4 rounded-2xl bg-gradient-to-r from-blue-950/30 via-slate-950 to-orange-950/30 border border-blue-500/30 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] bg-blue-500/20 text-blue-300 px-2 py-0.5 rounded-full font-mono">INR ₹ Gateway</span>
              </div>
              ...
              <div>
                <label className="text-[11px] text-slate-400 font-semibold block mb-1">Enter VPA / UPI ID (e.g., yourname@okhdfcbank):</label>
                <input
                  type="text"
                  value={upiId}
            ...
            {!isUserRegistered ? (
              <div className="p-4 rounded-2xl bg-gradient-to-r from-amber-500/10 via-rose-500/10 to-purple-500/10 border border-amber-500/30 space-y-2 text-xs">
                ...
                <p className="text-slate-300 leading-relaxed text-[11px]">
                  You are browsing as a guest. To ensure your payment details, transaction receipts, and refill credits are saved securely to your account, please create an account or log in first.
                </p>
                <button
                  type="button"
            ...
            >
              {!isUserRegistered ? (
                ...
                <>
                  ...
                  Pay {formattedPrice} with {gateway.name} & Refill
                </>
              )}
            </button>

            {/* Developer Integration Callout Link */}
            <div className="pt-2 border-t border-slate-800/80">
              <button
                type="button"
                onClick={() => setShowDeveloperGuide(!showDeveloperGuide)}
                className="w-full text-center text-[11px] text-blue-400 hover:text-blue-300 font-bold flex items-center justify-center gap-1.5 cursor-pointer py-1"
              >
                <Code className="w-3.5 h-3.5 text-blue-400" />
                <span>{showDeveloperGuide ? 'Hide' : 'How to Link Real Razorpay Keys (Integration Guide)'}</span>
              </button>

              {showDeveloperGuide && (
                <div className="mt-3 p-4 bg-slate-950 border border-blue-500/30 rounded-2xl text-[11px] text-slate-300 space-y-3 font-sans">
                  <div className="flex items-center gap-2 font-bold text-blue-300 text-xs border-b border-slate-800 pb-2">
                    <Terminal className="w-4 h-4 text-blue-400" />
                    <span>Razorpay Official Express Setup Instructions</span>
                  </div>

                  <ol className="list-decimal list-inside space-y-1.5 text-slate-300">
                    <li>
                      <strong>Step 1: Set Keys in Environment Variable:</strong>
                      <p className="pl-4 text-slate-400 font-mono text-[10px]">RAZORPAY_KEY_ID="rzp_live_xxxxxxxxxxxx"<br />RAZORPAY_KEY_SECRET="your_secret_key"</p>
                    </li>
                    <li>
                      <strong>Step 2: Add Script in index.html:</strong>
                      <p className="pl-4 text-slate-400 font-mono text-[10px]">&lt;script src="https://checkout.razorpay.com/v1/checkout.js"&gt;&lt;/script&gt;</p>
                    </li>
                    <li>
                      <strong>Step 3: Trigger Razorpay Modal in React:</strong>
                      <div className="mt-1 p-2 bg-slate-900 rounded-lg font-mono text-[9.5px] text-emerald-300 overflow-x-auto">
                        {`const options = {
...
rzp.open();`}
            ...
            <div className="pt-2 border-t border-slate-800/80 text-center space-y-2">
              ...
              <p className="text-center text-[10px] text-slate-500 flex items-center justify-center gap-1">
                ...
                Instant Credit Synchronization • 256-Bit SSL Encrypted Razorpay Checkout
              </p>
            </div>
          </div>
